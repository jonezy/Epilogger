    ...
    public partial class EventsController : BaseController
    {
        ...
        public virtual ActionResult CreateEvent(CreateBasicEventViewModel model)
        {
            ...
            if (EndDateValid(startDate, endDate))
                ...
                model.EndDateTime = null;   // end date was 
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    foreach (string s in searchTerms)
                    {
                        ...
                        {
                            if (s.Trim().Contains(' '))
                                searchQuery += quote + s.Trim() + quote + " OR ";
                            else
                                searchQuery += s + " OR ";
                        }
                    }
                    ...
                    //var ceEvent = Mapper.Map<Event,CreateEventViewModel>(eventModel);
                    //////The the admins an email with the event details.
                    //SendEventCreatedEmailToSystem(ceEvent);
        ...
        private string getEventStartEndTime(DateTime startDateTime, DateTime? endDateTime)
        {
            ...
            string totalDate = startDateTime.ToString("MMM dd hh:mm tt");
            string endDate = "";
            try
            {
                if (endDateTime.HasValue)
                {
                    if (DateTime.Compare(startDateTime, endDateTime.Value) <= 0)
                    {
                        endDate = endDateTime.Value.ToString("MMM dd hh:mm tt");
                        totalDate += " - " + endDate;
                    }
                }
            }
        ...
        public bool Delete(int eventID)
        {
            ...
            var deleteMSG = new MQ.Messages.SystemProcessingMSG { EventID = eventID, Task = MQ.Messages.SystemMessageType.Delete };
